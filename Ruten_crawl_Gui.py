...
=======
=======
>>>>>>> d48713a26f4fc85013dac7b5db1afa877173e142

stop_or_not = 0;
def process(result,Input_user):
    global stop_or_not
    stop_or_not=1
    browser = webdriver.Chrome(r'C:\Users\Luhung\Desktop\chrome\chromedriver.exe')
    browser.get('https://www.ruten.com.tw/store/'+Input_user.get("1.0","end")+'/')
    index = 1
    all_item = {}
    item_index = 0
...
=======
           print(str(item_index)+"_"+e.get_attribute('textContent').strip().split("\n")[0]+"_"+e.get_attribute('textContent').strip().split("\n")[1].strip()+" "+e.get_attribute('textContent').strip().split("\n")[2].strip())
        if index<134:
>>>>>>> d48713a ('程式碼')
=======
           print(str(item_index)+"_"+e.get_attribute('textContent').strip().split("\n")[0]+"_"+e.get_attribute('textContent').strip().split("\n")[1].strip()+" "+e.get_attribute('textContent').strip().split("\n")[2].strip())
...
=======
        f = open(r'C:\Users\Luhung\Desktop\first\data.txt', 'w+', encoding='UTF-8')
>>>>>>> d48713a ('程式碼')
=======
        f = open(r'C:\Users\Luhung\Desktop\first\data.txt', 'w+', encoding='UTF-8')
>>>>>>> d48713a26f4fc85013dac7b5db1afa877173e142
...
def start(result,Input_user):
    global stop_or_not
    messagebox.showinfo('Warning', 'Start searching')
    if stop_or_not ==0:
      result.delete('1.0', 'end')
      t = threading.Thread(target=process,args=(result,Input_user))
      t.start()
    else:
      print('wait')
...
def end():
    global stop_or_not
    messagebox.showinfo('Warning', 'Stop searching')
    stop_or_not = 0
    print(stop_or_not)   
<<<<<<< HEAD
>>>>>>> d48713a ('程式碼')
...
=======
>>>>>>> d48713a26f4fc85013dac7b5db1afa877173e142
  Input_user_label.place(x=98,y=50)
  Input_user = tk.Text(window,height=1,width=15)
  Input_user.place(x=200,y=52)
  Result= ScrolledText.ScrolledText(window, height=28,width=68,undo=True)
  Result.place(x=10,y=90)
  start_button = tk.Button(window,text = 'Start Search',command = lambda:start(Result,Input_user),height = 1,width = 20)
  start_button.place(x=10,y=10)
  stop_button = tk.Button(window,text = 'Stop Search',command = end,height = 1,width = 20)
  stop_button.place(x=350,y=10)
